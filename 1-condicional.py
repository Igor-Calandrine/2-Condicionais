...
Se quiser determinar se dois valores não são iguais, você poderá combinar um ponto de exclamação e um sinal de igualdade (!=). O ponto de
exclamação representa não, como ocorre em muitas linguagens de programação."""
...
"""Esse teste deve devolver True, independentemente do modo como o valor 'Audi' estiver formatado, pois o teste agora ignora as diferenças entre letras maiúsculas e minúsculas. A função lower() não altera o valor originalmente
armazenado em car, portanto você pode fazer esse tipo de comparação sem afetá-lo"""

print(carro.lower() == "audi")

"""Expressões booleanas
À medida que aprender mais sobre programação, você ouvirá o termo expressão booleana em algum momento. Uma expressão booleana é apenas outro nome para um teste condicional. Um valor booleano é True ou False,
exatamente como o valor de uma expressão condicional após ter sido avaliada.
